...
const startAIResponse = (sessionId, userMessage) => {
    ...
    fetchEventSource('/api/psychological-chat/stream', {
        ...
        onopen: async (response) => {
            ...
            // 非流式响应（通常是业务错误），读取并抛出以走错误分支，避免无限重连
            const text = await response.text().catch(() => '')
            throw new Error(text || '服务器返回非流式数据')
        },
        onmessage: (event) => {
        ...
        onerror: (err) => {
            // 抛出错误以阻止 fetch-event-source 默认自动重连
            handleError(err?.message || 'AI回复失败')
            activeCtrl = null
            throw err
        },
        onclose: () => {
            // 流正常关闭；情绪分析统一在 done 事件中触发，避免重复请求
            isAiTyping.value = false
            activeCtrl = null
        }
    })
...
const handleError = (message) => {
    ...
    if (aiMessage) {
        ...
        aiMessage.content = aiMessage.content
            ? `${aiMessage.content}\n\n（连接中断：${message || 'AI回复失败，请重试'}）`
            : 'AI回复失败，请重试'
    }
    isAiTyping.value = false
    ElMessage.error(message || 'AI回复失败，请重试')
}
...
const handleSessionClick = (session) => {
    if (isAiTyping.value) {
        ElMessage.warning('AI助手正在回复中，请稍候')
        return
    }
    // 点击会话时，获取会话详情
...
const handleDeleteSession = (sessionId) => {
    deleteSession(sessionId).then(() => {
        ...
        // 若删除的是当前会话，重置为新会话
        if (currentSession.value && ("session_" + sessionId) === currentSession.value.sessionId) {
            createNewFrontendSession()
            messages.value = []
        }
        getSessionPage()
