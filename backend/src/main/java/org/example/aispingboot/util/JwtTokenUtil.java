...
public class JwtTokenUtil implements ApplicationContextAware {
    ...
    public static String extractTokenFromRequest(HttpServletRequest request) {
        ...
        JwtConfig jwtConfig = getJwtConfig();
        String configuredHeader = jwtConfig.getHeader();
        String tokenPrefix = jwtConfig.getTokenPrefix();

        String tokenHeader = null;
        if (StringUtils.hasText(configuredHeader)) {
            tokenHeader = request.getHeader(configuredHeader);
        }
        ...
        if (StringUtils.hasText(tokenPrefix) && tokenHeader.startsWith(tokenPrefix)) {
            return tokenHeader.substring(tokenPrefix.length()).trim();
        }
