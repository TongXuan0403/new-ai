...
| 版本 | 分支 | 说明 |
| --- | --- | --- |
| V1.x（MVP） | `main` | 单体结构（`server` / `web` 两个目录），含 P0 安全 MVP 与 P1 连续使用增强 |
| **V2.0（本仓库）** | `v2.0` | **前后端独立工程**（`ai-spingboot` + `ai-vue`），引入 **Spring AI ChatClient + MCP（模型上下文协议）**，AI 对话可实时检索心理健康知识库，并新增后台数据分析 / 情绪日志管理等能力 |
