...
- **知识库检索增强（RAG）**：知识库数据源为 **MySQL 后台已发布文章**（`knowledge_article`），与「知识文章管理」完全打通——管理后台上传 / 编辑 / 发布后 **AI 立即检索到**（缓存自动失效，无需重启）；通过 `@Tool` 暴露为 **MCP 工具**（搜索 / 详情 / 分类），AI 在对话中自主调用检索，回答优先引用知识库内容；旧静态知识库启动时自动迁移入库。
