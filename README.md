...
- **知识库**：分类树 + 文章分页、富文本安全渲染（前端剥标签 + 转义）、内容审核流程（草稿 / 待审 / 已发布 / 下线）、推荐仅来自已审核内容。
- **管理后台**：运营概览（聚合脱敏）、知识内容管理、危机资源配置、风险事件中心、审计日志。
...
当前状态：冒烟 32/32 全部通过；安全回归 8 项 + 同意门 + 越权全部通过。
...
- **审计**：管理端文章 / 危机资源增改删、用户数据导出均写审计日志（`operator / action / target`）。
...
- 🔜 **P1 提高连续使用**（部分完成）：对话帮助度反馈聚合、日记编辑/删除/导出已落地；待做文章搜索/收藏、自助练习库、提示词/模型/规则版本化。
