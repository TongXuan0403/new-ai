...
const viewSessionDetail = (row) => {
    ...
    const defaultAi = { primaryEmotion: '', emotionScore: 0, riskLevel: '', isNegative: false, suggestion: '', riskDescription: '', improvementSuggestions: [] }
    if (row.aiEmotionAnalysis) {
        try {
            aiData.value = { ...defaultAi, ...JSON.parse(row.aiEmotionAnalysis) }
        } catch (e) {
            aiData.value = defaultAi
        }
    } else {
        aiData.value = defaultAi
    }
